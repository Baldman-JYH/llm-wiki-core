...
`llm-wiki-core` 是一个面向本地 LLM Wiki 工作流的中性实践项目。它把 [Karpathy 的 LLM Wiki 思想](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) 落成可测试、可维护的 Markdown 知识库工具，让本地智能体围绕原始资料持续维护 Wiki，而不是把知识留在一次性对话里。

当前版本状态：R3.1 在 MVP 本地闭环之上增加了 local Markdown 批量摄取。URL ingest、HTML cleanup、vector search、LLM synthesis、deep retrieval 等完整 R3 能力仍属于后续工作；本项目 does not claim full parity with `claude-obsidian`。
...
- Karpathy gist 提供的是 abstract idea，也就是 LLM Wiki 的抽象思想。
- [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) 是 Claude Code + Obsidian 工作流下的参考实现。
- `llm-wiki-core` 是 neutral、可测试、可迁移的 practice implementation，用来沉淀跨本地智能体可复用的核心规则。

`llm-wiki-core` 关注这套机制里的中性部分：
...
- 批量摄取 `.raw/` 下的本地 `.md` 文件，同时保留原始来源与 manifest 可追踪性。
- 从 Wiki 页面中检索并回答问题，返回 wikilink 引用。
- 保存长期有价值的结论到 Wiki。
- 通过 `status` 和 `continue` 恢复跨会话上下文。
- 检查 frontmatter、死链、孤立页面和基础结构健康。
- 为所有命令提供可选 `--json` 输出，便于脚本和本地智能体解析。
- 为 Codex App 和 Codex CLI 生成 `AGENTS.md` 使用入口。
- 使用 `filesystem` 作为稳定、可移植的运行通道。
- 支持 official `obsidian` CLI 的可选运行通道；只有在 vault 绑定与 capability probe 全部验证通过后才启用，否则继续使用 filesystem fallback。
...
如果 `llm-wiki` 命令暂时不在当前 shell 的 `PATH` 中，可以使用等价模块入口：
...
所有命令都可以追加 `--json`，输出单个机器可读 JSON 对象。这个模式适合 Codex App、Codex CLI、持续集成脚本或其他本地自动化流程解析结果。
...
成功结果会包含 `operation` 和 `status` 等字段；失败结果会返回 `status: "error"`，并在 `error.type` 与 `error.message` 中说明错误类型和原因。
...
| `llm-wiki ingest-batch <vault> <source-root>` | 摄取 `.raw/` 根目录下的本地 `.md` 文件 |
...
- [R1 稳定性加固报告](docs/r1-hardening-report.md)
...
当前版本聚焦本地 MVP 与 R3.1 batch ingest，不包含以下能力：

- official `obsidian` CLI 仅在验证通过后作为可选运行通道启用，未验证时继续使用 filesystem fallback。
- 不宣称与 `claude-obsidian` 的完整能力对齐。
- 尚未实现 URL ingest、HTML cleanup、image ingest、deep retrieval、vector search、LLM synthesis。
...
- R1：已完成，增强测试夹具、CLI 输出、frontmatter 校验和 `.raw/` 错误提示。
- R2：已完成，official `obsidian` CLI 通道采用 verified-only 策略并保留 filesystem fallback。
- R3：在已完成的本地 Markdown batch ingest 之上，继续扩展 URL ingest、deep retrieval 和更深的检索能力。
